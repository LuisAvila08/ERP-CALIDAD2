...
import { API, SPECIAL_PRICES, ALL } from '@/config/Constant'
...
import { SpecialPricesInterface } from '@/interfaces/SpecialPrices'
...
export function DataTable<TData, TValue> ({ columns, data }: DataTableProps<TData, TValue>): JSX.Element {
  ...
  const [specialPrices, setSpecialPrices] = useState<SpecialPricesInterface[]>([])
  ...
  useEffect(() => {
    axios.get(`${API}${SPECIAL_PRICES}${ALL}`).then((response) => {
      setSpecialPrices(response.data.specialPrices)
    }).catch((_error) => {
      setSpecialPrices([])
    })
  }, [reload])

  const renderSubRows = (row: Row<TData>): ReactNode => {
    const filteredSpecialPrices = specialPrices.filter((specialPrice: SpecialPricesInterface) => specialPrice.Articulo === (row.original as any).Articulo)
    const shouldRenderSubRow = filteredSpecialPrices.length > 0

    return shouldRenderSubRow
      ? (
          filteredSpecialPrices.map((specialPrice: SpecialPricesInterface) => {
            return (
              <TableRow key={specialPrice.SPID}>
                <TableCell />
                <TableCell colSpan={columns.length - 1} className='md:pl-[100px]'>
                  SPID: <strong className='text-secondary'>{specialPrice.SPID}</strong> | <strong>{specialPrice.SerieLote}</strong> | <span className='text-primary'><strong>{moneyFormat(specialPrice.Precio)}</strong></span>
                  {specialPrice.Nota?.trim().length > 0 && (<strong> | {specialPrice.Nota}</strong>)}
                </TableCell>
              </TableRow>
            )
          })
